...
const QuizPlayScreen = ({
  ...
  if (questions.length === 0) {
    return <div className="p-4 pb-20 space-y-6 min-h-screen flex items-center justify-center">
  ...
  if (quizCompleted) {
    return <div className="p-4 pb-20 space-y-6 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 min-h-screen">
        <div className="text-center space-y-4">
          <div className="w-20 h-20 bg-green-600 rounded-full flex items-center justify-center mx-auto my-[50px]">
        ...
        <div className="space-y-3">
          <button onClick={() => onNavigate('home')} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-4 rounded-xl font-semibold transition-colors">
            Back to Home
          </button>
          <button onClick={() => onNavigate('quiz-configuration')} className="w-full bg-slate-700 hover:bg-slate-600 text-white py-4 rounded-xl font-semibold transition-colors">
            Take Another Quiz
          </button>
        </div>
      </div>;
  ...
  return <div className="p-4 pb-20 space-y-6 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 min-h-screen">
      ...
      <div className="flex flex-wrap gap-2">
        {currentQ.subjects.map(subject => <span key={subject} className="bg-blue-600/20 text-blue-300 px-2 py-1 rounded-full text-xs">
            {subject}
          </span>)}
        {currentQ.systems.map(system => <span key={system} className="bg-green-600/20 text-green-300 px-2 py-1 rounded-full text-xs">
            {system}
          </span>)}
      </div>
      ...
      <div className="space-y-4">
        ...
        <RadioGroup value={selectedAnswer?.toString() || ""} onValueChange={value => handleAnswerSelect(parseInt(value))} disabled={showResult} className="space-y-3">
          {currentQ.options.map((option, index) => <div key={index} className={`w-full p-4 rounded-xl text-left transition-colors ${selectedAnswer === index ? showResult ? index === currentQ.correctAnswer ? 'bg-green-600 border-green-500 text-white' : 'bg-red-600 border-red-500 text-white' : 'bg-blue-600 border-blue-500 text-white' : showResult && index === currentQ.correctAnswer ? 'bg-green-600 border-green-500 text-white' : 'bg-slate-800 hover:bg-slate-700 border-slate-700 text-white'} border`}>
        ...
        {showResult && currentQ.explanation && <div className="bg-slate-800 rounded-xl p-4 border-l-4 border-blue-500">
      ...
      {/* Next Button */}
      {selectedAnswer !== null && !showResult && <button onClick={handleNext} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-4 rounded-xl font-semibold transition-colors">
