...
const GrandLessonScreen = ({
  onNavigate,
  onGrandLessonSelect
}: GrandLessonScreenProps) => {
  ...
  return <div className="p-4 pb-20 space-y-6 min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      ...
      <div className="flex items-center space-x-3">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-white my-[40px]">
            {selectedLessonData ? selectedLessonData.title : "Grand Lessons"}
          </h1>
          <p className="text-slate-300 text-sm">
            {selectedLessonData ? selectedLessonData.description : "Comprehensive learning paths for medical mastery"}
          </p>
        </div>
      ...
      {selectedLessonData ? <GrandLessonDetail grandLesson={selectedLessonData} onBack={() => setSelectedGrandLesson(null)} onModuleClick={handleModuleClick} getDifficultyColor={getDifficultyColor} /> : <>
          <LearningProgressHeader />
          <div className="space-y-4">
            {grandLessons.map(grandLesson => <GrandLessonCard key={grandLesson.id} grandLesson={grandLesson} onClick={handleGrandLessonClick} />)}
          </div>
        </>}
    </div>;
};
