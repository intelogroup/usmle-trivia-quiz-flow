...
const AnalyticsScreen = ({ onNavigate }: AnalyticsScreenProps) => {
  ...
  const { readinessScore, topicPerformance, sessionAnalytics } = analytics;

  // Get recent performance data
  const recentSessions = sessionAnalytics.slice(0, 7);
  const avgRecentAccuracy = Math.round(
    recentSessions.reduce((sum, session) => sum + session.accuracy, 0) / recentSessions.length
  );

  // Get subject performance for table
  const subjectPerformance = Object.entries(readinessScore.subjectReadiness)
    .map(([subject, score]) => {
                      ...
                      <TableCell>
                        <div className={`flex items-center space-x-1 ${
                          ...
                          <TrendingUp className={`w-3 h-3 ${
                            subject.improvement < 0 ? 'rotate-180' : ''
                          }`} />
