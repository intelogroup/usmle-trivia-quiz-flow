...
const SettingsScreen = ({
  ...
  return <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white">
      ...
      <div className="flex items-center justify-between p-4 border-b border-slate-700/50 bg-slate-800/50 backdrop-blur-sm">
        <button onClick={() => onNavigate('profile')} className="flex items-center space-x-2 text-slate-400 hover:text-white transition-colors">
          <ChevronLeft className="w-6 h-6" />
          <span>Back</span>
        </button>
        <h1 className="text-xl font-bold my-[30px]">Settings</h1>
      ...
      <div className="p-4 pb-20 space-y-6">
        ...
        <div className="space-y-6">
          {settingsGroups.map((group, groupIndex) => <div key={groupIndex} className="space-y-3">
              ...
              return <button key={itemIndex} onClick={item.action} className={`w-full p-4 flex items-center justify-between hover:bg-slate-700/50 transition-all duration-200 border-b border-slate-600/30 last:border-b-0 ${item.destructive ? 'hover:bg-red-500/10' : ''}`}>
                      ...
                      <div className="flex items-center space-x-2">
                        {item.hasToggle ? <Switch checked={item.enabled} onCheckedChange={item.onToggle} className="data-[state=checked]:bg-blue-600" /> : item.hasArrow ? <ChevronRight className={`w-5 h-5 ${item.destructive ? 'text-red-400' : 'text-slate-400'}`} /> : null}
                      </div>
                    </button>;
            })}
              </div>
