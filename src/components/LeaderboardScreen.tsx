...
const LeaderboardScreen = () => {
  ...
  const pointsToNextRank = currentUser && currentUser.rank > 1 ? leaderboardData[currentUser.rank - 2].points - currentUser.points : 0;
  return <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-900 to-slate-800 p-4 pb-20">
      {/* Header */}
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold text-white mb-2 bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent my-[40px]">
          Leaderboard
        </h1>
      ...
      {/* Top 3 Podium */}
      {podiumOrder.length === 3 && <div className="mb-8">
          <div className="flex justify-center items-end gap-4 mb-6">
            {podiumOrder.map((player, index) => {
          const isFirst = player.rank === 1;
          const cardHeight = isFirst ? "h-36" : "h-28";
          const avatarSize = isFirst ? "w-16 h-16" : "w-12 h-12";
          return <div key={player.rank} className="flex flex-col items-center cursor-pointer hover:scale-105 transition-all duration-300 group" onClick={() => handleUserClick(player)}>
                  {/* Avatar */}
      ...
      <div className="bg-gradient-to-br from-slate-800/80 to-slate-700/80 backdrop-blur-sm rounded-lg border border-slate-600/30 overflow-hidden shadow-xl">
        ...
        <div className="divide-y divide-slate-600/30">
          {leaderboardData.map(player => <div key={player.rank} className={`flex items-center justify-between px-4 py-4 cursor-pointer transition-all duration-200 hover:scale-[1.02] ${player.isCurrentUser ? 'bg-blue-900/50 hover:bg-blue-800/60 border-l-4 border-blue-400' : 'hover:bg-slate-700/50'}`} onClick={() => handleUserClick(player)}>
              <div className="flex items-center gap-4">
                {/* Rank */}
                <div className="w-8 flex justify-center">
                  {player.rank <= 3 ? getRankIcon(player.rank) : <span className="text-sm font-semibold text-slate-400">{player.rank}</span>}
                </div>
                ...
                <div className="relative group">
                  <Avatar className="w-10 h-10 ring-2 ring-slate-600/50 transition-all duration-200 group-hover:ring-blue-400/50">
                    <AvatarImage src="" />
                    <AvatarFallback className={`text-white font-medium transition-all duration-200 ${player.isCurrentUser ? 'bg-gradient-to-br from-blue-500 to-blue-600' : 'bg-gradient-to-br from-slate-600 to-slate-700'}`}>
                      {player.avatar}
                    </AvatarFallback>
                  </Avatar>
                ...
                <div className="flex-1">
                  <div className={`font-semibold transition-colors duration-200 ${player.isCurrentUser ? 'text-blue-300' : 'text-white'}`}>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {player.strongestSubjects?.slice(0, 2).map((subject, index) => <Badge key={subject.subject} variant="outline" className="text-xs px-2 py-0.5 text-green-400 border-green-400/30 bg-green-400/10 hover:bg-green-400/20">
      ...
      {currentUser && <div className="mt-6 bg-gradient-to-r from-slate-800/80 to-slate-700/80 backdrop-blur-sm rounded-lg border border-blue-500/30 p-4 shadow-xl">
          <div className="text-center">
            ...
            <div className="flex items-center justify-center gap-4">
              ...
              <div className="text-sm text-blue-300">
                ...
                {pointsToNextRank > 0 && <div className="text-blue-400">{pointsToNextRank.toLocaleString()} points to rank up</div>}
              </div>
            </div>
          </div>
        </div>}
